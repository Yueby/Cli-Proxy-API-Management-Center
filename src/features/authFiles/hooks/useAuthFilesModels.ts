...
export function useAuthFilesModels(): UseAuthFilesModelsResult {
  ...
  const [, setModelsCacheRenderVersion] = useState(0);
  ...
  const getCachedModels = useCallback(
    (name: string) => modelsCacheRef.current.get(name.trim()),
    []
  );
  ...
  const invalidateModels = useCallback((names?: string[]) => {
    if (!names) {
      ...
      setModelsCacheRenderVersion((version) => version + 1);
      return;
    }
  ...
  const prefetchModels = useCallback(
    async (item: AuthFileItem) => {
      if (modelsCacheRef.current.has(item.name.trim())) return;
      try {
  ...
  const showModels = useCallback(
    async (item: AuthFileItem) => {
      ...
      try {
        ...
        if (isCacheCurrent() && isRequestCurrent()) setModelsList(models);
      } catch (err) {
