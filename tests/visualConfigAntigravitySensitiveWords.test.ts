...
describe('visual config Antigravity sensitive words', () => {
  ...
  test('indexes antigravitySensitiveWords for visual config search in system section', () => {
    const searchEntry = CONFIG_FIELD_SEARCH_INDEX.find(
      (entry) => entry.fieldId === 'antigravitySensitiveWords'
    );

    expect(searchEntry).toBeDefined();
    expect(searchEntry?.sectionId).toBe('system');
    expect(searchEntry?.yamlKeys).toEqual(['antigravity', 'sensitive-words']);
    expect(searchEntry?.keywords).toEqual(
      expect.arrayContaining(['antigravity', 'obfuscate', 'zero-width'])
    );
  });
});
