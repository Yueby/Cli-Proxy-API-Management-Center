...
import { formatCredentialWeightBadge } from '../src/utils/credentialWeight';
...
describe('auth-file credential weight patch', () => {
  test('formats an explicit card badge and hides the implicit default', () => {
    expect(formatCredentialWeightBadge(0)).toBe('W0');
    expect(formatCredentialWeightBadge(-2)).toBe('W-2');
    expect(formatCredentialWeightBadge(5)).toBe('W5');
    expect(formatCredentialWeightBadge(undefined)).toBeNull();
    expect(formatCredentialWeightBadge(1.5)).toBeNull();
  });
