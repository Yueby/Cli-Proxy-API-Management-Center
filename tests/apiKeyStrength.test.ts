...
describe('API key strength', () => {
  ...
  test('periodic keys score like a single period', () => {
    const repeatedDeadbeef = evaluateApiKeyStrength('deadbeefdeadbeefdeadbeefdeadbeef');
    expect(repeatedDeadbeef.tier).toBe('weak');
    expect(repeatedDeadbeef.bits).toBe(evaluateApiKeyStrength('deadbeef').bits);
  ...
  test('repeating a non-strong period never promotes it to strong', () => {
    for (const period of ['abcde', 'A1-bC', 'proxy2024', 'aZ7!qP2@wL9#xR4']) {
      expect(evaluateApiKeyStrength(period.repeat(100)).tier).not.toBe('strong');
      expect(evaluateApiKeyStrength(`${period.repeat(100)}${period.slice(0, 3)}`).tier).not.toBe(
        'strong'
      );
    }
  });
  ...
  test('guessable tokens drag the score down', () => {
    const withToken = evaluateApiKeyStrength('sk-password-Xk7#mQ2vLp9$Rn4wZt6cHj8&Bd');
    const withoutToken = evaluateApiKeyStrength('sk-hRv6Qtz-Xk7#mQ2vLp9$Rn4wZt6cHj8&Bd');
